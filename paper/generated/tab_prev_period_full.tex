\begin{table*}[t]
\centering\footnotesize
\begin{tabular}{lrrrcr}
\toprule
period & $N$ & $k$ & \% & 95\% CI & per 100k words \\
\midrule
pre-1990 & 368,842 & 58,577 & 15.88 & 15.76--16.00 & 7.9 \\
1990-2004 & 257,715 & 54,071 & 20.98 & 20.82--21.14 & 10.1 \\
2005-2014 & 180,721 & 35,908 & 19.87 & 19.69--20.05 & 10.3 \\
2015-2026 & 120,976 & 27,054 & 22.36 & 22.13--22.60 & 10.7 \\
\bottomrule
\end{tabular}
\caption{The same quantity by era. The rate per hundred thousand words is reported alongside the opinion-level rate because opinion length is not constant across eras.}
\label{tab:prevperiod}
\end{table*}
